	...
	int *keep, double *rkeep, double *cg_dginit)
{
	...
	// 	LogConsole::writeConsole("---%d eval", *nfev);
...
L30: if (brackt)
{
	 ...
	 *stp = std::min<double>(*stp, *stpmax);
	 ...
	 /*finish the first time line search*/
	 //LogConsole::writeConsole("%.8f", *stp);
	 *info = -1;
	 goto L1010;
L45: *info = 0;
	 ++(*nfev);
	 ...
	 /*if preconditioned Conjugate gradient*/
	 if (cg_dginit)
	 {
	 ...
	 {
		 //TNPACK's C2 version.
		 //if (*f <= ftest1 && (dg >= *gtol * dginit || dg <= (2.0 - (*gtol)) * dginit)) {
			// LogConsole::writeConsole("TNPACK's C2");
			// *info = 1;
		 //}
