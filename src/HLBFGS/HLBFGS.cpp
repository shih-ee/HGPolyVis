...
void LINESEARCH_MESSAGE(int info, int iternum)
{
	switch (info)
	{
	case 6:
		LogConsole::writeConsole("[LSearch fail %d!] too small step(%d)", info, iternum);
		break;
		break;
	case 5:
		LogConsole::writeConsole("[LSearch fail %d!] get biggest step(%d)", info, iternum);
		break;
	case 4:
		LogConsole::writeConsole("[LSearch fail %d!] get smallest step(%d)", info, iternum);
		break;
	case 3:
		LogConsole::writeConsole("[LSearch fail %d!] exceed linesearch IterMax(%d)", info, iternum);
		break;
	case 2:
		LogConsole::writeConsole("[LSearch fail %d!] too small bracket(%d)", info, iternum);
		break;
	default:
		LogConsole::showError("invalid linesearch return %d(%d)", info, iternum);
		break;
	}
}

//////////////////////////////////////////////////////////////////////////
void HLBFGS_MESSAGE(bool print, int id, int iterNum, const double PARAMETERS[])
{
	if (!print)
	{
		return;
	...
	{
	case 0:
		LogConsole::writeConsole("Please check your input parameters (%d)!", iterNum);
		break;
	case 1:
		LogConsole::writeConsole("Linesearch failed (%d)!", iterNum);
		break;
	case 2:
		LogConsole::writeConsole("Convergence (%d): ||g||/max(1,||x||) <= %.16f", iterNum, PARAMETERS[5]);
		break;
	case 3:
		LogConsole::writeConsole("Convergence (%d): ||g|| <= %.3e", iterNum, PARAMETERS[6]);
		break;
	case 4:
		LogConsole::writeConsole("Convergence(%d): linesearch cannot improve anymore", iterNum);
		break;
	case 5:
		LogConsole::writeConsole("Exceeds max iteration (%d)", iterNum);
		break;
	default:
		break;
	}
...
//////////////////////////////////////////////////////////////////////////
bool HLBFGS(int N, int M, double* x,
	...
	bool NEWITERATION(int, int, double*, double*, double*, double*, double*),
	double PARAMETERS[],
	int INFO[],
	double* searchDir,
	double* step
)
{
	...
	{
		HLBFGS_MESSAGE(INFO[5] != 0, 0, 0, PARAMETERS);
		return false;
	}
	//allocate mem
	...
	//parameter[1] = 1.0e-8;	//variable tolerance used in line-search, 1.0e-16
	//parameter[2] = 0.9;		//gradient tolerance used in line-search, 0.9
	//parameter[3] = 1.0e-20;	//stpmin used in line-search, 1.0e-20
	//parameter[4] = 1.0;		//stpmax used in line-search, 1.0e+20
	...
	do
	{
		...
		//M is the number of LBFGS updates (typical choices: 3-20)
		if (INFO[2] > 0 && M > 0)
		{
			//compute s and y
			start = cur_pos * N;
...
#pragma omp parallel for private(i)
			...
			//INFO[13] 0: standard lbfgs update, 1: Biggs's update, 2: Yuan's update; 3: Zhang and Xu's update
			if (INFO[13] == 1)
			{
...
#pragma omp parallel for private(i)
		...
		else  //Enable preconditioned Conjugate Gradient; 
		{
			//with accurate hessian
			if (INFO[7] == 1)
			{
				ICFS_INFO& l_info = m_hessian.get_icfs_info();
				dstrsol_(&N, l_info.get_l(), l_info.get_ldiag(),
...
#pragma omp parallel for private(i)
		...
		//INFO[2] the current number of iterations, M is the number of LBFGS updates (typical choices: 3-20)
		if (INFO[2] > 0 && M > 0)
		{
		...
		/*line search return, 
		if info is -1, linesearch continue, 
		if info is 1, linesearch success,
		else, linesearch fails
		*/
		info = 0;
		...
		do
		{
			/*do the binary linesearch maxfev times to find bracked region*/
			...
			//INFO[9], 0: classical line-search; 1: modified line-search (it is not useful in practice)
		...
		gnorm = HLBFGS_DNRM2(N, g);
		INFO[2]++;
		bool keepOpt = NEWITERATION(INFO[2], INFO[1], x, &f, g, &gnorm, q);
		...
		if (info != 1)
		{
			LINESEARCH_MESSAGE(info, INFO[2]);
			return false;
		}
		if (gnorm / xnorm <= PARAMETERS[5])
		{
			HLBFGS_MESSAGE(INFO[5] != 0, 2, INFO[2], PARAMETERS);
			return true;
		}
		if (gnorm < PARAMETERS[6])
		{
			HLBFGS_MESSAGE(INFO[5] != 0, 3, INFO[2], PARAMETERS);
			return true;
		}
		if (stp < stpmin || stp > stpmax)
		{
			LogConsole::writeConsole("stp:%f, stpmin:%f, stpmax:%f", stp, stpmin, stpmax);
			HLBFGS_MESSAGE(INFO[5] != 0, 4, INFO[2], PARAMETERS);
			return false;
		}
		if (INFO[2] >= INFO[4])
		{
			HLBFGS_MESSAGE(INFO[5] != 0, 5, INFO[2], PARAMETERS);
			return false;
		}
