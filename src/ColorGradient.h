...
public:
	ColorGradient() { createRainbowHeatMapGradient(); }
	~ColorGradient() {
		clearGradient();
	}
	...
	void createRainbowHeatMapGradient()
	{
		color.clear();
	...
	void createRedBlueHeatMapGradient()
	{
		color.clear();
		color.push_back(ColorPoint(0, 0, 1, 0.0f));      // Blue.
		color.push_back(ColorPoint(1, 0, 0, 1.0f));      // Red.
	...
	void createRedWhiteBlueHeatMapGradient() 
	{
		color.clear();
		color.push_back(ColorPoint(0, 0, 1, 0.0f));      // Blue.
		color.push_back(ColorPoint(1, 1, 1, 0.5f));      // white.
		color.push_back(ColorPoint(1, 0, 0, 1.0f));      // Red.
	}


	void createRedGreenBlueHeatMapGradient()
	{
		color.clear();
		color.push_back(ColorPoint(0, 0, 1, 0.0f));      // Blue.
		color.push_back(ColorPoint(0, 1, 0, 0.5f));      // green.
	...
	void createColorRamp(int n, const Color4_8BIT *colors) {
		color.clear();
		double offset = 1.0 / (n - 1);
		for (int i = 0; i < n; i++) {
			Color4_8BIT colori = colors[i];
			float r = colori.R / 255.0;
			float g = colori.G / 255.0;
			float b = colori.B / 255.0;
			color.push_back(ColorPoint(r, g, b, i*offset));
		}
	}
